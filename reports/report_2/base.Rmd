...
What is interesting here is the fact that $\text{Cor}(\texttt{age}, \texttt{npreg})$ and $\text{Cor}(\texttt{skin}, \texttt{bmi})$ are the most correlated amongst the covariates, with $\text{Cor}(\texttt{age}, \texttt{bp})$ following somewhat behind.
...
\begin{align*}
  \vec{B} &= [10, 20, 30, 40, 50] \\
  \vec{d} &= [1, 3, 5, 10] \\
  \vec{\lambda} &= [0.2, 0.15, 0.1, 0.05, 0.01] \\
  \vec{m} &= [10, 15, 20, 30]
\end{align*}
...
tune_grid <- expand.grid(
 n.trees = c(10, 20, 30, 40, 50),
 interaction.depth = c(1, 3, 5, 10),
 shrinkage = c(0.2, 0.15, 0.1, 0.05, 0.01),
...
Here it becomes obvious that $\lambda = 0.01$ is the worst of them all.
...
$B = 40$, $d = 5$, $\lambda = 0.1$, and $m = 15$ performs best, so this is what is chosen in the final model.
...
The output activation function, $\phi_o$, must also be chosen and we use the sigmoid function yet again, $\phi_o = \phi_h$.
