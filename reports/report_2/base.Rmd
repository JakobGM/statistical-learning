...
A `price` prediction for the covariate vector $\boldsymbol{x}_0$ is constructed in the following manner

$$
\widehat{\texttt{logprice}}
=
\boldsymbol{\hat{\beta}}^T \boldsymbol{x}_0 \\
\implies
\widehat{\texttt{price}}
=
10^{\boldsymbol{\hat{\beta}}^T \boldsymbol{x}_0} = 10^{\beta_0} \cdot \prod_{i = 1}^{19} 10^{\beta_i \boldsymbol{x}_{0,i}}.
$$
...
All other covariates must be interpreted *relative to* this "reference diamond". For instance, since $10^{\hat{\beta}_{\texttt{cutIdeal}}} \approx 1.135$, we would expect a diamond with an *ideal cut* to cost 13.5\% more than the "reference diamond", all other properties being equal.
...
The idea of a regression tree is to take $n$ observation pairs $(\boldsymbol{x}_i, Y_i)$ and separating them into $J$ partitions of the predictor space, $R_1, ..., R_J$.
...
A prediction, $\hat{f}_{R_j}$, for $x_0$ belonging to the region $R_j$, is therefore calculated as
...
The idea is to draw a sufficiently large amount, $B$, bootstrap replicates from the training set and fit a tree, $\hat{f}_{b}(\boldsymbol{x})$, for every single replicate, $X_b$.
The resulting predictor, $\hat{f}_{\text{avg}}(\boldsymbol{x})$ is the *average* of all those trees,

$$
\hat{f}_{\text{avg}}(\boldsymbol{x}) = \frac{1}{B} \sum_{b = 1}^{B} \hat{f}_b (\boldsymbol{x}).
$$
...
**Q21:** What is the definition of a principal component score, and how is the score related to the eigenvectors of the matrix \({\hat {\boldsymbol R}}\). 

Given the estimated covariance matrix \( \hat{\boldsymbol R} \), we define its $m$-th principal component, $\boldsymbol{W}_m \in \mathbb{R}^{p \times 1}$, as

\begin{equation}
  \label{eq:decomposition}
  \boldsymbol{W}_m
  =
  \hat{\boldsymbol{R}} \boldsymbol{\phi}_m
  =
  \sum_{j = 1}^p \phi_{jm} \hat{\boldsymbol R}_j,
  ~~~~\text{subject to}~~~~||\phi_{m}||_{2} = 1
\end{equation}

We want $\mathrm{Var}(\boldsymbol{W}_i) \geq \mathrm{Var}(\boldsymbol{W}_{i + 1})$ and $\boldsymbol{W}_i \perp \boldsymbol{W}_j$ given $i \neq j$. The intent here is to linearly decompose the estimated covariance matrix into uncorrelated columns ordered by their contribution to the overall variance in the data. That way we can transform the variables into a space of lower dimension while retaining the greatest possible amount of information.

Since $\hat{\boldsymbol{R}}$ is a non-negative definite matrix it has an eigen-decomposition

$$
\hat{\boldsymbol{R}} = \boldsymbol{C} \Lambda \boldsymbol{C}^{-1},
$$

where $\Lambda = \text{diag}(\lambda_1, ..., \lambda_p)$ is the diagonal eigenvalue matrix in \textit{decreasing} order and $\boldsymbol{C}$ is a matrix with the \textit{eigenvectors} of $\hat{\boldsymbol R}$ for its columns.

It can be shown that $\boldsymbol{\phi} = [\phi_1, ..., \phi_p]^T = \boldsymbol{C}$ solves equation \eqref{eq:decomposition}.

The *principal component scores* are the new values we attain in this transformed space.
The $j$th principal component score for covariate $i$ is therefore

$$
  \boldsymbol{W}_{ij} = (\boldsymbol{W} \boldsymbol{C})_{ij}.
$$
...
Figure \ref{fig:eigenvector} shows the values of of the first eigenvector of $\hat{\boldsymbol{R}}$.
With other words, it shows the entries of $\phi_1$, how to linearly combine the columns of $\hat{\boldsymbol{R}}$ in order to get the first principal component, which explains the most variance of all the principal components.

Since $\boldsymbol{X}$ and $\boldsymbol{Z}$ are $n \times p = 6380 \times 64$ matrices, there are only $64$ eigenvectors and eigenvalues.

**Q23:** How many principal components are needed to explain 80% of the total variance in ${\boldsymbol{Z}}$? Why is `sum(pca$sdev^2)=p`? 

If we transform $\boldsymbol{Z}$ into the first $M$ principal components, the fraction of the original variance that is kept is
...
```{r}
library(ggplot2)
pca_data <- pca$x %>% as_tibble()

our_colors <- as.character(colsvsnames[,1])
names(our_colors) <- our_colors

pca_data %>%
  ggplot() +
  geom_point(
    aes(x = PC1, y = PC2, color = colsamples)
  ) + 
  scale_color_manual(values = our_colors, labels = colsvsnames[,2])
```
