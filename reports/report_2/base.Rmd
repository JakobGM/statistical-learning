...
Given the estimated covariance matrix \( \hat{\mat R} \), we define its $m$-th principal component, $\mat{W}_m \in \mathbb{R}^{p \times 1}$, as
...
We want $\mathrm{Var}(\mat{W}_i) \geq \mathrm{Var}(\mat{W}_{i + 1})$ and $\mat{W}_i \perp \mat{W}_j$ given $i \neq j$. The intent here is to linearly decompose the estimated covariance matrix into uncorrelated columns ordered by their contribution to the overall variance in the data. That way we can transform the variables into a space of lower dimension while retaining the greatest possible amount of information.

Since $\hat{\mat{R}}$ is a non-negative definite matrix it has an eigen-decomposition

$$
\hat{\mat{R}} = \mat{C} \mat{\Lambda} \mat{C}^{-1},
$$

where $\mat{\Lambda} = \text{diag}(\lambda_1, ..., \lambda_p)$ is the diagonal eigenvalue matrix in \textit{decreasing} order and $\mat{C}$ is a matrix with the \textit{eigenvectors} of $\hat{\mat R}$ for its columns.

It can be shown that $\mat{\phi} = [\phi_1, ..., \phi_p]^T = \mat{C}$ solves equation \eqref{eq:decomposition}.
