...
The pixels that are crossed out are considered to have an insignificant correlation at a significance level of $0.05$.
What is interesting here is the fact that only $\text{Cor}(\texttt{age}, \texttt{npreg})$ and $\text{Cor}(\texttt{skin}, \texttt{bmi})$ are considered to be significant.
`diabetes` has no significant correlation to any of the covariates, which can be considered bad for our predictive capabilities using this data set.
