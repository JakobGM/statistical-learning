...
Given the estimated covariance matrix \( \hat{\bf R} \), we define its $m$-th principal component, $\bf{W}_m \in \mathbb{R}^{p \times 1}$, as
...
\begin{equation}
  ...
  \hat{\bf{R}} \bf{\phi}_m
  =
  \sum_{j = 1}^p \phi_{jm} \hat{\bf R}_j,
  ~~~~\text{subject to}~~~~||\phi_{m}||_{2} = 1
\end{equation}

We want $\mathrm{Var}(\bf{W}_i) \geq \mathrm{Var}(\bf{W}_{i + 1})$ and $\bf{W}_i \perp \bf{W}_j$ given $i \neq j$. The intent here is to linearly decompose the estimated covariance matrix into uncorrelated columns ordered by their contribution to the overall variance in the data. That way we can transform the variables into a space of lower dimension while retaining the greatest possible amount of information.

Since $\hat{\bf{R}}$ is a non-negative definite matrix it has an eigen-decomposition

$$
\hat{\bf{R}} = \bf{C} \Lambda \bf{C}^{-1},
$$

where $\Lambda = \text{diag}(\lambda_1, ..., \lambda_p)$ is the diagonal eigenvalue matrix in \textit{decreasing} order and $\bf{C}$ is a matrix with the \textit{eigenvectors} of $\hat{\bf R}$ for its columns.

It can be shown that $\bf{\phi} = [\phi_1, ..., \phi_p]^T = \bf{C}$ solves equation \eqref{eq:decomposition}.
...
Figure \ref{fig:eigenvector} shows the values of of the first eigenvector of $\hat{\bf{R}}$.
With other words, it shows the entries of $\phi_1$, how to linearly combine the columns of $\hat{\bf{R}$ in order to get the first principal component, which explains the most variance of all the principal components.

Since $\bf{X}$ and $\bf{Z}$ are $n \times p = 6380 \times 64$ matrices, there are only $64$ eigenvectors and eigenvalues.
