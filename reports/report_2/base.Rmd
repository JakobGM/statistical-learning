...
An lastly, we plot `PC5` against `PC6`.

```{r}
plot_pca(5, 6)
```

This is the plot with the least visual patterns so far.
Most of the points are placed around $\texttt{PC6} = -\texttt{PC5}$, `LEUKEMIA` seems to be spread far away on both sides of this line.
