...
title: "Compulsory exercise 2: Group 64"
...
AIC achieves this by using RSS as the measure of fit while \textit{adding} a complexity term in order to adjust for overfitting.
The entirety of the data is used for the measure.

Cross-validation with mean squared test error MSE does not require such an additional complexity term.
It rather splits the data into training and validation sets, trains the model on the training set, and calculates the MSE with the validation test.
That way it implicitly guards against overfitting since the validation set is not part of the model training procedure.
...
Logistic regression requires independence between observational pairs $(Y_i, \vec{x}_i)$.
...
\begin{align*}
  ...
  &=
  \prod \limits_{i = 1}^{n}
  p\left(\vec{x}_i \mid \vec{\beta}\right)^{y_i}
  (1 - p\left(\vec{x}_i \mid \vec{\beta})\right)^{1 - y_i}
\end{align*}

are chosen.

As in linear regression, we need to decide on the number of parameters to include in our model.
One way to decide on which subset of the $p$ available predictors to include is best subset selection.
This is performed exhaustively below with $\text{AIC}$ as selection criterion.
...
\begin{equation*}
  \log \left( 
  \frac{\hat{p}(\vec{x}_i)}{1 - \hat{p}(\vec{x}_i)} 
  \right)
  =
  \vec{x}_i^T \hat{\vec{\beta}},
\end{equation*}

which shows that the log odds is linear in the predictors when using the sigmoid function.
This means that by increasing one of the predictors $x_{ij}$ by one unit, while holding everything else constant, the predicted log odds will increase by $\hat{\beta}_j$ units.
Equivalently, the odds will increase by a factor of $e^{\hat{\beta}_j}$.
The linear relation between our independent variables and tho log odds is one of our model assumptions.
...
Under the Bayes classifier in a two-class problem an observation $\vec{x}_i$ is assigned the class for which the predicted posterior is greater than 0.5. Using this, we can evaluate the performance of the model on the test set. The results are shown below, along with a plot of the $\text{ROC}$-curve and the corresponding $\text{AUC}$.
...
\begin{itemize}
  ...
  \item The shrinkage parameter, $\lambda$, determines how fast the the algorithm learns. A lower value will require more trees in order to offset the slow learning. \texttt{gbm} calls this parameter \texttt{shrinkage}.
...
$B = `r boost_model$bestTune[[1]]`$, $d = `r boost_model$bestTune[[2]]`$, $\lambda = `r boost_model$bestTune[[3]]`$, and $m = `r boost_model$bestTune[[4]]`$ performs best, so this is what is chosen in the final model.
...
The result here is an overall accuracy of $\approx 78.5\%$, i.e. a misclassification rate of $21.5\%$.
Interestingly, we have a $\approx 89\%$ sensitivity, so the model is able to correctly predict diabetes for nine out of ten diabetes sufferers.
...
Here we see that the ROC curve is far from being diagonal and $\text{AUC} \approx 0.84 > 0.5$, which a random guessing model would have.
It can therefore be considered to have explaining power.
...
We therefore have a misclassification rate of $\approx 20\%$.
...
We define an objective function, $J(\vec{\theta})$, a scaled version of the *binomial cross-entropy loss*, which is the negative of the binomial log likelihood in scaled form
...
We seek to find the $\vec{\theta}$ which minimizes this loss function.
...
We will perform a grid search over the tuning parameter space $\vec{w} \times \vec{\lambda} = [1, 2, ..., 11] \times [0, 0.1, ..., 0.5]$ with the same method as explained before.
...
We can now perform this grid search and retrieve the best model.
The covariates need not really be centered and scaled, but it often helps with the convergence rate of the optimization algorithm, as explained in [this StackOverflow answer](https://stackoverflow.com/a/4674770/11009311).
...
By inspection we see that `r nn_model$bestTune[[1]]` hidden nodes with a weight decay of `r nn_model$bestTune[[2]]` is optimal, which can be confirmed:
...
This is hard to parse, so we will use a plot method implemented by the GitHub user \texttt{Peque} [avaliable here](https://gist.github.com/Peque/41a9e20d6687f2f3108d) in order to visualize the neural network:
...
Grey edges represent negative weights, while the black ones represent positive weights.
The thickness of each line is proportional to the magnitude of the weights.
...
We can now see that the logistic and svm models have comparable accuracies of $\approx 79.7\%$, but with different sensitivities and specificities.
