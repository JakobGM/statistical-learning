...
```{r}
test_mse <- mean((dtest$logprice - predict.lm(fit, dtest))^2)
test_mse
```
...
test_mse <- mean((dtest$logprice - predictions) ^ 2)
print(test_mse)
```
...
library(tree)
full_tree <- tree(
  logprice~logcarat+cut+clarity+color+depth+table+xx+yy+zz-1,
  data = dtrain
)
```
...
This results in the trees becoming highly correlated, and we lose much of the variance reduction which we expect from the averaging of the trees. A *random forest* aims to solve this by randomly removing a fraction of the covariates in each split, instead of using *all* the covariates as we do in *bagging*. This fraction is often set to $m \approx \sqrt{p}$ for classification trees, and $m = p / 3$ for regression trees.
