...
We now plot the decision boundary of $\tilde{y}(x) = \underset{k}{\mathrm{argmax}}$, which is simply the line $x_1 = x_2$.
...
Hence, we prefer the argmax predictor since it has (barely) better overall accuracy.
...
So the result is that $\hat{\beta} \sim N_{p + 1}(\beta, \sigma^2 (X^T X)^{-1})$, $p$ being the number of covariates in the model.
...
Asymptotically, the graph reflects this prediction, but there is a "dip" in the region $\sim [0.1, 0.5]$, which is not exactly what we expected.
...
This is what we would expect, from the fact that as $\lambda$ increases, the design matrix is relatively less weighted.
This is in accordance with the bias-variance trade-off.
...
The optimal $\lambda$ is the one which minimizes the total MSE, which in this case is $\lambda \approx 0.994$.
